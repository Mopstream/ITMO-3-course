...
##########################################################
create or replace function worker_add_f()
returns trigger as $$
    begin
        insert into worker_family_member(EMP_ID, ENTRY_DATE)
 VALUES (NEW.ID, current_timestamp);
 RETURN NEW;
    end;
$$ language plpgsql;

create trigger worker_add_t
after insert on worker execute function worker_add_f();
########################################################

следить за ревенью чтобы влиять на работников(манагеров) <1000 ревенью -100_000 > rent + summ salary




хитро кладем новую траву...
автомат карточки на членов семьи (манагер работник и лэндлорд)
